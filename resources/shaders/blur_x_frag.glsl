...
uniform vec2 InvAORes = 0.5 / vec2(1024.0, 768.0);
...
{
	ivec2 aoCoord = ivec2(round(TexCoord * AORes + uv*InvAORes));
	ivec2 dCoord = ivec2(round(TexCoord * FullRes + uv * InvFullRes));

	return vec2(texelFetch(texture0, aoCoord, 0).r,
